...
describe("PM-Skills", () => {
  ...
  test("should execute growth loops skill", async () => {
    ...
    const loops = result.loops as Record<string, unknown>;
    assert.ok(loops);
  ...
  test("should execute product roadmap skill", async () => {
    ...
    const roadmap = result.roadmap as Record<string, unknown>;
    assert.ok(roadmap);
  ...
  test("should handle invalid skill names", async () => {
    await assert.rejects(
      () => pmSkills.executeSkill("invalidSkill", {}),
      /Unknown skill/,
    );
  });
